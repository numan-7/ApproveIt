...
import { Paperclip, Download, Pencil, X, Users, FileText, Calendar } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="">Requested by: {approval.requester}</p>
          </div>
...
interface ApprovalStatusCardProps {
  approval: Approval;
  user: any;
  currentUserApprover?: { email: string; didApprove: boolean | null; name: string };
  onEdit: () => void;
  onDelete: () => void;
  onApprove: () => void;
  onDeny: () => void;
            ...
            <>
              {currentUserApprover && (
                <>
                  ...
                  >
                    {currentUserApprover.didApprove === false ? 'Rejected' : 'Reject'}
                  </Button>
                </>
              )}
            </>
